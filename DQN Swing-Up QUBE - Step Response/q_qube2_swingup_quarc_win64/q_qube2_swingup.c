 ...
 * C source code generated on : Fri Mar 11 15:11:48 2022
 *
...
void q_qube2_swingup_output(void)
{
    ...
    /* S-Function Block: q_qube2_swingup/Rotary Pendulum Interface/HIL Read Encoder Timebase (hil_read_encoder_timebase_block) */
    {
      ...
      if (result < 0) {
        ...
        rtb_HILReadEncoderTimebase_o1 =
          q_qube2_swingup_DW.HILReadEncoderTimebase_Buffer[0];
        rtb_Abs = q_qube2_swingup_DW.HILReadEncoderTimebase_Buffer[1];
      }
    }
  }
  ...
  if (rtmIsMajorTimeStep(q_qube2_swingup_M)) {
    ...
    q_qube2_swingup_B.Pendulumcountstorad =
      q_qube2_swingup_P.Pendulumcountstorad_Gain * rtb_Abs;

    /* SignalConversion generated from: '<Root>/Vector Concatenate' */
    rtb_VectorConcatenate[1] = q_qube2_swingup_B.Pendulumcountstorad;
  }
  ...
  if (rtmIsMajorTimeStep(q_qube2_swingup_M)) {
    ...
    for (k = 0; k < 4; k++) {
       ...
       *  SignalConversion generated from: '<Root>/Vector Concatenate'
       */
      rtb_Product1 += rtb_VectorConcatenate[k] * (q_qube2_swingup_P.Q[k + 12] *
        q_qube2_swingup_B.RateTransition1 + (q_qube2_swingup_P.Q[k + 8] *
        rtb_VectorConcatenate[2] + (q_qube2_swingup_P.Q[k + 4] *
        rtb_VectorConcatenate[1] + q_qube2_swingup_P.Q[k] *
        rtb_VectorConcatenate[0])));
    }

    /* Product: '<S1>/Matrix Multiply1' */
    q_qube2_swingup_B.MatrixMultiply1 = rtb_Product1;
  }
   ...
   *  Step: '<Root>/Step1'
   */
  rtb_Product = q_qube2_swingup_M->Timing.t[0];
  if (rtb_Product < q_qube2_swingup_P.Step2_Time) {
    rtb_Product1 = q_qube2_swingup_P.Step2_Y0;
  } else {
    rtb_Product1 = q_qube2_swingup_P.Step2_YFinal;
  }
   ...
   */
  rtb_Product1 *= q_qube2_swingup_P.SetpointStateXd2_Value;

  /* Step: '<Root>/Step1' */
  if (rtb_Product < q_qube2_swingup_P.Step1_Time) {
    rtb_Product = q_qube2_swingup_P.Step1_Y0;
  } else {
    rtb_Product = q_qube2_swingup_P.Step1_YFinal;
  }

  /* Product: '<Root>/Product' incorporates:
   *  Constant: '<Root>/Setpoint State: Xd1'
   */
  rtb_Product *= q_qube2_swingup_P.SetpointStateXd1_Value;

  /* Sum: '<Root>/Sum3' */
  rtb_MatrixMultiply4 = (rtb_Product1 + q_qube2_swingup_B.Armcountstorad) -
    rtb_Product;
   ...
   */
  rtb_Reshape[0] = sin(rtb_MatrixMultiply4);
  rtb_Reshape[1] = cos(rtb_MatrixMultiply4);
  if (rtmIsMajorTimeStep(q_qube2_swingup_M)) {
    /* Trigonometry: '<Root>/Sin1' */
    q_qube2_swingup_B.Sin1 = sin(q_qube2_swingup_B.Pendulumcountstorad);

    /* Trigonometry: '<Root>/Cos1' */
    q_qube2_swingup_B.Cos1 = cos(q_qube2_swingup_B.Pendulumcountstorad);
  }

  /* Reshape: '<Root>/Reshape' */
  rtb_Reshape[2] = q_qube2_swingup_B.RateTransition;
  rtb_Reshape[3] = q_qube2_swingup_B.Sin1;
  rtb_Reshape[4] = q_qube2_swingup_B.Cos1;
  ...
  for (iy = 0; iy < 24; iy++) {
    for (k = 0; k < 6; k++) {
      x[k] = q_qube2_swingup_P.layer1[24 * k + iy] * rtb_Reshape[k];
    }

    rtb_MatrixMultiply4 = x[0];
    for (k = 0; k < 5; k++) {
      rtb_MatrixMultiply4 += x[k + 1];
    }

    values3[iy] = fmax(0.0, rtb_MatrixMultiply4 + q_qube2_swingup_P.layer2[iy]);
  }

  for (iy = 0; iy < 24; iy++) {
    for (k = 0; k < 24; k++) {
      b_x[k] = q_qube2_swingup_P.layer3[24 * k + iy] * values3[k];
    }

    rtb_MatrixMultiply4 = b_x[0];
    for (k = 0; k < 23; k++) {
      rtb_MatrixMultiply4 += b_x[k + 1];
    }

    values2[iy] = fmax(0.0, rtb_MatrixMultiply4 + q_qube2_swingup_P.layer4[iy]);
  }

  for (iy = 0; iy < 3; iy++) {
    for (k = 0; k < 24; k++) {
      b_x[k] = q_qube2_swingup_P.layer5[3 * k + iy] * values2[k];
    }
    ...
    values6[iy] = rtb_MatrixMultiply4 + q_qube2_swingup_P.layer6[iy];
  }
  ...
  if (iy == 0) {
    ...
    rtb_MatrixMultiply4 = values6[iy - 1];
    for (k = iy; k < 3; k++) {
      if (rtb_MatrixMultiply4 < values6[k]) {
        rtb_MatrixMultiply4 = values6[k];
        iy = k + 1;
      }
    }
  }

  q_qube2_swingup_B.action = ((real_T)iy - 2.0) * q_qube2_swingup_P.tau_max;
   ...
   */
  q_qube2_swingup_B.Sum = q_qube2_swingup_P.R * q_qube2_swingup_B.action *
    q_qube2_swingup_B.action + q_qube2_swingup_B.MatrixMultiply1;

  /* Sum: '<Root>/Sum' */
  q_qube2_swingup_B.Sum_o = rtb_Product - rtb_Product1;
  if (rtmIsMajorTimeStep(q_qube2_swingup_M)) {
  }

  /* Gain: '<Root>/Gain' */
  rtb_Product1 = 10.0 / q_qube2_swingup_P.tau_max * q_qube2_swingup_B.action;

  /* Saturate: '<Root>/+//-10V' */
  if (rtb_Product1 > q_qube2_swingup_P.u0V_UpperSat) {
    /* Saturate: '<Root>/+//-10V' */
    q_qube2_swingup_B.u0V = q_qube2_swingup_P.u0V_UpperSat;
  } else if (rtb_Product1 < q_qube2_swingup_P.u0V_LowerSat) {
    /* Saturate: '<Root>/+//-10V' */
    q_qube2_swingup_B.u0V = q_qube2_swingup_P.u0V_LowerSat;
  } else {
    /* Saturate: '<Root>/+//-10V' */
    q_qube2_swingup_B.u0V = rtb_Product1;
  }
  ...
  if (rtmIsMajorTimeStep(q_qube2_swingup_M)) {
    ...
    rtb_Abs = rt_modd_snf(rtb_Abs, q_qube2_swingup_P.Constant2_Value);

    /* Bias: '<S4>/Bias2' */
    q_qube2_swingup_B.Bias2 = rtb_Abs + q_qube2_swingup_P.Bias2_Bias_a;
     ...
     */
    q_qube2_swingup_B.Gain4 = q_qube2_swingup_B.Bias1 * q_qube2_swingup_B.Bias1 *
      q_qube2_swingup_P.Gain4_Gain;

    /* Gain: '<S7>/Gain' incorporates:
     *  Math: '<S7>/Math Function1'
     */
    q_qube2_swingup_B.Gain = q_qube2_swingup_B.Bias2 * q_qube2_swingup_B.Bias2 *
      q_qube2_swingup_P.Gain_Gain;

    /* Gain: '<S7>/Gain5' incorporates:
     *  Math: '<S7>/Math Function3'
     */
    q_qube2_swingup_B.Gain5 = q_qube2_swingup_B.RateTransition1 *
      q_qube2_swingup_B.RateTransition1 * q_qube2_swingup_P.Gain5_Gain;

    /* Gain: '<S7>/Gain3' incorporates:
     *  Math: '<S7>/Math Function4'
     */
    q_qube2_swingup_B.Gain3 = q_qube2_swingup_B.RateTransition *
      q_qube2_swingup_B.RateTransition * q_qube2_swingup_P.Gain3_Gain;
  }
   ...
   */
  q_qube2_swingup_B.Add = ((((0.0 - q_qube2_swingup_B.Gain4) -
    q_qube2_swingup_B.Gain) - q_qube2_swingup_B.Gain5) - q_qube2_swingup_B.Gain3)
    - q_qube2_swingup_B.action * q_qube2_swingup_B.action *
...
RT_MODEL_q_qube2_swingup_T *q_qube2_swingup(void)
{
  ...
  /* External mode info */
  q_qube2_swingup_M->Sizes.checksums[0] = (2664191423U);
  q_qube2_swingup_M->Sizes.checksums[1] = (3939533595U);
  q_qube2_swingup_M->Sizes.checksums[2] = (2393985380U);
  q_qube2_swingup_M->Sizes.checksums[3] = (1989316544U);
  ...
  q_qube2_swingup_M->Sizes.numBlocks = (92);/* Number of blocks */
  q_qube2_swingup_M->Sizes.numBlockIO = (25);/* Number of block outputs */
  q_qube2_swingup_M->Sizes.numBlockPrms = (955);/* Sum of parameter "widths" */
