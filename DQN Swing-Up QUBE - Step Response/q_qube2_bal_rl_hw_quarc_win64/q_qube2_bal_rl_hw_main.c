...
typedef RT_MODEL_q_qube2_bal_rl_hw_T RT_MODEL;

/*====================*
 * External functions *
 *====================*/
EXTERN RT_MODEL * q_qube2_bal_rl_hw(void);
  ...
  _do_assertion(const char * expression, const char * file_name, int line_number)
{
  ...
  rtmSetErrorStatus(q_qube2_bal_rl_hw_M, GBLbuf.message);
}
  ...
  control_c_handler(int signal_number)
{
  ...
  GBLbuf.stopExecutionFlag = 1;
  rtExtModeStart();
  hil_task_stop(q_qube2_bal_rl_hw_DW.HILReadEncoderTimebase_Task)
    ;
}
  ...
  main(int argc, char * argv[])
{
  ...
  if (parseError) {
    (void) fprintf(stderr,
                   "\nUsage: q_qube2_bal_rl_hw -option1 val1 -option2 val2 -option3 ...\n\n");
    (void) fprintf(stderr,
  ...
  if (rtExtModeQuarcStartup(rtmGetRTWExtModeInfo(S),
    ...
    if (rtmGetErrorStatus(S) == NULL) {
      ...
      fflush(stdout);
      result = hil_task_start(q_qube2_bal_rl_hw_DW.HILReadEncoderTimebase_Task,
        (t_clock) q_qube2_bal_rl_hw_P.HILReadEncoderTimebase_Clock, 100.0, -1)
        ;
      if (result == 0) {
